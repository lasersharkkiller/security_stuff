...
# Script to submit a file, no password, and specify network exits. Replace $key $currentfile and $networkexit

###ThreatGrid API key, set network exit (i.e. jp-ven = Tokyo Japan)
...
$currentfile = "C:\bits\2019 Salaries.xlsx"
		...
		$Body = (
			...
			$key,
            "------------MULTIPARTBOUNDARY_`$",
			'Content-Disposition: form-data; name="ntework_exit"',
			"",
			$networkexit,
			"------------MULTIPARTBOUNDARY_`$",
