...
#### Getting Time Range #####
$cur_time = Get-Date
$time_to_look_from = $cur_time.AddHours(-78) #Set the number of hours back for query. If on cron job how many hours between runs.
...
$currentfile = $files[$i].FullName
    ...
    Expand-7Zip -ArchiveFileName $currentfile -Password "INFECTEDfiles" -TargetPath "C:\BITS\temp\tempfolder\"
    Compress-7Zip -Path C:\BITS\temp\tempfolder\ -ArchiveFileName C:\BITS\temp\$newcurrentfile -Format Zip -Password "INFECTEDfiles"
